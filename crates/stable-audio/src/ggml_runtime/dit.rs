...
impl GgmlWeights {
    ...
    unsafe fn dit_block(
        ...
        let mut v = ffi::ggml_reshape_3d(ctx, v, HEAD_DIM, N_HEAD, n_tokens);
        q = ffi::ggml_mul(
            ctx,
        ...
        attn = ffi::ggml_cont_2d(ctx, attn, EMBED, n_tokens);
        ...
        let mut q = ffi::ggml_mul_mat(ctx, self.tensor(&(p.to_owned() + "ca.q.weight"))?, h);
        ...
        kq = ffi::ggml_soft_max(ctx, kq);
        v = ffi::ggml_cont(ctx, ffi::ggml_transpose(ctx, v));
        let mut ca = ffi::ggml_mul_mat(ctx, v, kq);
        ca = ffi::ggml_permute(ctx, ca, 0, 2, 1, 3);
        ca = ffi::ggml_cont_2d(ctx, ca, EMBED, n_tokens);
        ...
        let value = ffi::ggml_cont(ctx, ffi::ggml_view_2d(ctx, h, 4096, n_tokens, row_bytes, 0));
        let gate = ffi::ggml_cont(
            ctx,
            ffi::ggml_view_2d(ctx, h, 4096, n_tokens, row_bytes, 4096 * 4),
        );
